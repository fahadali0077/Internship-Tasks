...
export function RegisterForm() {
  ...
  /**
   * Uncontrolled file input — Module 5 task requirement.
   *
   * File inputs CANNOT be controlled (React cannot set their value).
   * useRef gives us access to the DOM node to read files on submit:
   *   avatarRef.current.files?.[0]  → the selected File object
   *
   * In Module 6+: pass this File to a Server Action that uploads to
   * Cloudinary / S3 and stores the URL in the user record.
   */
  ...
  const onSubmit = async (data: RegisterFormValues) => {
    ...
    console.log("Register:", data, "Avatar:", avatarFile?.name ?? "none");
  ...
  if (submitted) {
    return (
      <div className="flex flex-col items-center gap-4 py-8 text-center">
        <div className="flex h-16 w-16 items-center justify-center rounded-full bg-green-100 text-2xl text-green-600 dark:bg-green-900/30">✓</div>
      ...
      >
        ...
        <div className="flex flex-col gap-2">
          <span className="text-sm font-medium text-ink dark:text-white">
            Profile photo <span className="font-normal text-ink-muted">(optional)</span>
          </span>
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              <p className="text-[11px] text-ink-muted">
                ...
                <code className="rounded bg-cream px-1 dark:bg-dark-surface-2">avatarRef.current.files</code>
              </p>
            </div>
          </div>
        </div>
            ...
            {passwordValue.length > 0 && (
              <div className="mt-1.5 flex items-center gap-2">
                <div className="flex flex-1 gap-1">
                  {[1, 2, 3, 4].map((n) => (
                    <div key={n}
                      className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${n <= strength.level ? strength.color : "bg-border dark:bg-dark-border"}`}
                    />
                  ))}
                </div>
                {strength.label && <span className="text-xs font-medium text-ink-muted">{strength.label}</span>}
              </div>
            )}
            <FormMessage />
          </FormItem>
        ...
        <p className="text-center text-sm text-ink-muted">
          ...
          <Link href="/auth/login" className="font-semibold text-amber hover:underline">Log in →</Link>
        </p>
      </form>
    </Form>
  );
