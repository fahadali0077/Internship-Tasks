...
export function RegisterForm() {
  ...
  /**
   * Uncontrolled file input — useRef pattern (required by curriculum).
   *
   * File inputs cannot be controlled via React state — the browser
   * prevents setting their value programmatically for security reasons.
   * useRef gives us access to the DOM node to read files at submit time.
   *
   * Controlled input:   value={state}  onChange={setState}  — React owns the value
   * Uncontrolled input: defaultValue   ref.current.value    — DOM owns the value
   *
   * Read on submit: avatarRef.current.files?.[0]  → File object
   *
   * Module 8 / MERN-III upgrade: pass the File to a Server Action that
   * uploads to Cloudinary/S3 and stores the URL in the user MongoDB document.
   */
