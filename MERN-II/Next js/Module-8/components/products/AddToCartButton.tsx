...
/**
 * AddToCartButton — Module 6 version.
 *
 * MENTAL MODEL — Server Action vs Zustand:
 *   Module 5: onClick → useCartStore(addItem) → Zustand (in-memory, client-only)
 *   Module 6: onClick → startTransition → addToCart() Server Action
 *             → HTTP POST to Next.js → cookie updated → revalidatePath('/cart')
 *             → /cart Server Component re-renders with fresh data
 *
 *   We keep the Zustand store for the CartDrawer UI (instant client feedback).
 *   The cookie session is the source of truth for the /cart page (persists refresh).
 *   This dual-write pattern is intentional for this module.
 *
 * useTransition:
 *   isPending = true while the Server Action is in flight.
 *   We disable the button and show a spinner to prevent double-clicks.
 */
