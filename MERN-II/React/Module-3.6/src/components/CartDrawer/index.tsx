...
/**
 * CartDrawer — slides in from the right when cartStore.isOpen is true.
 *
 * NOTE: "use client" is NOT needed here. That directive only belongs in
 * Next.js App Router (Module 4+) to mark the server/client boundary.
 * In a Vite SPA everything is already client-side.
 *
 * Accessibility requirements for a modal drawer:
 *   role="dialog" + aria-modal="true"  → announces as a dialog to screen readers
 *   aria-label                          → names the dialog
 *   aria-hidden={!isOpen}               → hides from AT when closed
 *   Escape key closes the drawer        → standard modal keyboard contract
 *   Backdrop click closes the drawer    → pointer equivalent
 *
 * Badge consistency:
 *   Uses totalItems() (sum of all quantities) to match the navbar badge exactly.
 *   items.length is the number of distinct products — diverges when any qty > 1.
 */
export function CartDrawer() {
  ...
  // Escape key — standard modal keyboard contract
      ...
      >
        ...
        <div className="drawer-header">
          <h2 className="drawer-title">
            Cart
            {/* Badge matches navbar: totalItems() = sum of all quantities */}
            {totalItems > 0 && (
        ...
        {items.length > 0 && (
          <>
            ...
            <div className="drawer-footer">
              ...
              {/*
                Checkout disabled until Module 6 wires the Server Action.
                disabled + aria-label + title communicate the stub state clearly.
              */}
              <button
