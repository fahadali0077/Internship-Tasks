...
/**
 * RegisterForm — Module 3.5 deliverable.
 *
 * MENTAL MODEL — React Hook Form:
 *
 *   RHF uses an UNCONTROLLED strategy internally — it tracks field values
 *   in a ref (not state), so typing does NOT cause re-renders. The component
 *   only re-renders when:
 *     - validation errors appear/disappear
 *     - submission state changes (isSubmitting)
 *
 *   This is why RHF has "zero re-renders on typing" — compare it to a naive
 *   controlled form where every keystroke calls setState → re-render.
 *
 * KEY APIS:
 *   register()       — spreads { name, ref, onChange, onBlur } onto an input
 *   handleSubmit()   — wraps your submit handler, calls Zod schema first
 *   formState.errors — typed error messages per field (from Zod schema)
 *   formState.isSubmitting — true while async submit handler is running
 *
 * UNCONTROLLED FILE INPUT:
 *   <input type="file"> cannot be a controlled input in React (no value prop).
 *   We use useRef to read the FileList imperatively on submit.
 *   This is the canonical pattern for file inputs in React.
 *
 * ZOD INTEGRATION:
 *   zodResolver(RegisterSchema) — RHF calls Zod's safeParse on every submit
 *   (and optionally on every change for real-time feedback).
 *   The errors object is automatically typed based on RegisterSchema's shape.
 */
        ...
        {passwordValue.length > 0 && (
          <div className="strength-bar" aria-label={`Password strength: ${strength.label}`}>
            {[1, 2, 3, 4].map((n) => (
              <div
                key={n}
                className={`strength-segment ${n <= strength.level
                    ? `strength--${["", "weak", "fair", "good", "strong"][strength.level]}`
                    : ""
                  }`}
              />
            ))}
