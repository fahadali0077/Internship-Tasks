...
export const getAllProducts = asyncHandler(async (req: Request, res: Response) => {
  ...
  res.json({ success: true, data: products, message: `${products.length} products`, pagination: { page: pageNum, limit: limitNum, total, pages: Math.ceil(total / limitNum) } });
});
...
export const uploadProductImage = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!product) throw new AppError("Product not found", 404);
  res.json({ success: true, data: product, message: "Image uploaded to Cloudinary" });
});
